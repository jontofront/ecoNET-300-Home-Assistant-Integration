...
class ApiEndpointTester:
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                if isinstance(value, (dict, list)):
                    structure[key] = self.analyze_json_structure(value, max_depth, current_depth + 1)
                else:
    ...
    def save_results(self, results: Dict[str, Any], output_dir: str = "api_test_results"):
    ...
    def create_summary_report(self, results: Dict[str, Any], file_path: Path):
        ...
        with file_path.open('w', encoding='utf-8') as f:
            ...
            successful = [ep for ep, result in results["endpoints"].items() if result["status_code"] == 200]
            for endpoint in successful:
                result = results["endpoints"][endpoint]
                f.write(f"- **{endpoint}** ({result['response_time']}s, {result['response_size']} bytes)\n")
            f.write("\n")
            
            # Failed endpoints
            f.write("## ❌ Failed Endpoints\n\n")
            failed = [ep for ep, result in results["endpoints"].items() if result["status_code"] != 200]
            ...
            json_responses = [ep for ep, result in results["endpoints"].items() if result.get("is_json", False)]
            text_responses = [ep for ep, result in results["endpoints"].items() if result["status_code"] == 200 and not result.get("is_json", False)]
    ...
    def create_endpoint_documentation(self, results: Dict[str, Any], file_path: Path):
        ...
        with file_path.open('w', encoding='utf-8') as f:
            ...
            successful = {ep: result for ep, result in results["endpoints"].items() if result["status_code"] == 200}
            ...
            for endpoint, result in successful.items():
                ...
                f.write(f"- **Response Type:** {'JSON' if result['is_json'] else 'Text'}\n\n")
                ...
                if result.get("data") and isinstance(result["data"], dict) and len(result["data"]) < 10:
...
def main():
    ...
    parser.add_argument("--output-dir", default="api_test_results", help="Output directory for results")
